...
run2 m = maximum $ fromL ++ fromR ++ fromU ++ fromD
    where
        fromL = [run [((-1,y),R)] S.empty S.empty m | y <- [0..(length m)] ]
        fromR = [run [((length (head m),y),L)] S.empty S.empty m | y <- [0..(length m)] ]
        fromU = [run [((x,-1),D)] S.empty S.empty m | x <- [0..(length (head m))] ]
        fromD = [run [((x,length m),U)] S.empty S.empty m | x <- [0..(length (head m))] ]
